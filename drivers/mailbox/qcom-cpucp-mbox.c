...
static int qcom_cpucp_mbox_probe(struct platform_device *pdev)
{
	...
	cpucp->rx_base = devm_platform_ioremap_resource_byname(pdev, "rx");
	if (IS_ERR(cpucp->rx_base)) {
		if (PTR_ERR(cpucp->rx_base) != -EINVAL)
			return PTR_ERR(cpucp->rx_base);
		/* reg-names not present, fall back to index 0 */
		cpucp->rx_base = devm_of_iomap(dev, dev->of_node, 0, NULL);
		if (IS_ERR(cpucp->rx_base))
			return PTR_ERR(cpucp->rx_base);
	}

	cpucp->tx_base = devm_platform_ioremap_resource_byname(pdev, "tx");
	if (IS_ERR(cpucp->tx_base)) {
		if (PTR_ERR(cpucp->tx_base) != -EINVAL)
			return PTR_ERR(cpucp->tx_base);
		/* reg-names not present, fall back to index 1 */
...
static const struct of_device_id qcom_cpucp_mbox_of_match[] = {
	...
	{ .compatible = "qcom,nord-pdp-mbox", .data = &qcom_nord_mbox_data },
	{}
};
